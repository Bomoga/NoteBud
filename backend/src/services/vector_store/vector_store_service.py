    ...
    for batch in _batches(chunk_ids, _BATCH_SIZE):
                async with driver.session() as session:
                                for chunk_id in batch:
                                                    # Query the vector index for the top-K neighbours of this chunk.
                                                    # db.index.vector.queryNodes returns rows of (node, score).
                                                    # We skip the chunk itself (score == 1.0 / same id) and filter
                                                    # by the threshold before writing edges.
                                    ...
                                    # Collect qualifying pairs and write bidirectional SIMILAR edges
                                    # in a single batched Cypher statement for this chunk.
