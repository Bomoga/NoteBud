...
async def test_store_chunks_inserts_records(db_session):
    chunks = [
        {"notebook_id": 99901, "file_id": 1, "text": "Chunk one",
         "metadata": {"source_file": "test.pdf", "page_number": 1}},
        {"notebook_id": 99901, "file_id": 1, "text": "Chunk two",
...
async def test_store_chunks_raises_on_length_mismatch(db_session):
    chunks = [{"notebook_id": 99901, "text": "chunk"}]
    embeddings = []
...
async def test_similarity_search_returns_similar_chunks(db_session):
    ...
    chunks = [
        {"notebook_id": 99901, "file_id": 1, "text": "Similar chunk", "metadata": {}},
        {"notebook_id": 99901, "file_id": 1, "text": "Dissimilar chunk", "metadata": {}},
    ]
    embeddings = [similar, dissimilar]

    await store_chunks(db_session, chunks, embeddings)
    results = await similarity_search(db_session, base, notebook_id=99901, top_k=5)
...
async def test_similarity_search_scoped_to_notebook(db_session):
    ...
    chunks = [
        {"notebook_id": 99901, "file_id": 1, "text": "Notebook 1 chunk", "metadata": {}},
        {"notebook_id": 99902, "file_id": 2, "text": "Notebook 2 chunk", "metadata": {}},
    ]
    embeddings = [embedding, embedding]

    await store_chunks(db_session, chunks, embeddings)
    results = await similarity_search(db_session, embedding, notebook_id=99901, top_k=5)
...
async def test_similarity_search_respects_top_k(db_session):
    ...
    chunks = [
        {"notebook_id": 99901, "text": f"Chunk {i}", "metadata": {}}
        for i in range(10)
    ]
    embeddings = [embedding for _ in range(10)]

    await store_chunks(db_session, chunks, embeddings)
    results = await similarity_search(db_session, embedding, notebook_id=99901, top_k=3)
